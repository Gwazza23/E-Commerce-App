...
const getUserCart = (id) => {
    return new Promise((resolve,reject) => {
        pool.query("SELECT products.id, products.name, products.img_url ,cart.quantity,cart.price * cart.quantity AS total_price FROM cart,products WHERE products.id = cart.product_id AND user_id= $1", [id], (error,results) => {
            if(error){
                reject(error)
            }
